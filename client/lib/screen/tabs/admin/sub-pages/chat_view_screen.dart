...
class _ChatViewScreenState extends State<ChatViewScreen> {
  ...
  void initState() {
    super.initState();
    _loadMessages();
    // 🔁 Auto-refresh every 2 seconds (less frequent to avoid race conditions)
  ...
  Future<void> _loadMessages() async {
    try {
      ...
      final url = Uri.parse(
        "$baseUrl/messages/${widget.currentUserId}/${widget.userId}",
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            ...
            _messages.addAll(
              data.map((msg) {
                // Check if message is from current user by comparing sender_id
        ...
        print("Error loading messages: ${response.statusCode} - ${response.body}");
      }
    } catch (e) {
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      final url = Uri.parse("$baseUrl/messages/doctor");
      ...
      if (response.statusCode == 200) {
        ...
        // Reload messages immediately to get the latest from backend
  ...
  /// Send image (currently just sends file path)
