...
exports.sendForgotOtp = async (req, res) => {
	try {
		...
		await OTP.create({ user_id: user.user_id, code });

		// You could send it via email here
		return res.status(200).json({
			message: "OTP sent for password reset",
			code, // ⚠️ for testing only
		});
	} catch (error) {
...
exports.verifyForgotOtp = async (req, res) => {
	try {
		...
		const otp = await OTP.findOne({
			where: { user_id: user.user_id, code, status: "unused" },
			order: [["createdAt", "DESC"]],
		});
		...
		// Optional: issue a temporary token for resetting the password
